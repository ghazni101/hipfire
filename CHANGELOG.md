...
## Unreleased
- K2-Horizon-7B dense support (arch 16, AR): grouped-RMSNorm llama family — `LlamaConfig.norm_groups` with per-group affine norms (raw `layernorm_num_groups`, nested `rope_parameters.rope_theta`), LlamaCarrier claims arch id 16, ManifestPlainLlama load route. IFM think contract: `<|ifm|think>` markers route to the reasoning channel, `ReasoningContract::IfmThink` (toggle -> primer, max_think_tokens resolves, effort dropped), and the llama AR loop force-closes an unclosed think span (disabled thinking, or a finite budget >= 2 exhausted) so answers arrive instead of an empty-content max_tokens burn. Assistant history without a thinking field renders an empty `<ifm|think>` block instead of failing the Jinja render. Perf: batched chunk prefill admits all-MQ4G256V2 llama weights (TTFT no longer grows per-token with context), MQ4G256V2 x-batch GEMV (<= 8 rows) + WMMA arms at the four prefill projection sites, one-launch grouped RMSNorm batch kernel, GQA-fused Q8 tile decode attention (`attention_flash_q8_0_tile_gqa{2,4,8}`, `HIPFIRE_Q8_FLASH_GQA=0` opts out) and Llama-arch Q8 prefill attention family on gfx11/gfx1201. Verified gfx1101: coherent single/multi-turn chat, think-budget force-close, 8.1k-token prefill 700 tok/s, decode 60.4 tok/s short / 50.9 tok/s @ 8k ctx (Q8 KV).
