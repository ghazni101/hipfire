...
## Unreleased
- K2-Horizon MoVA-36B-A4B (`K2-Horizon-MoVA-36B-A4B.mq4`, arch_id 16) gains AR decode support: ported `hipfire-arch-k2-horizon` (MoVA attention — 64 sigmoid-routed value experts, top-4, softplus post-attn gate — + sigmoid MoE FFN: 100 experts top-8 + 1 shared; grouped RMSNorm) from the `merge/k2-horizon-pack-mq4g256v2` line. Dense and MoVA k2_horizon files share arch_id 16; `LlamaCarrier` disambiguates on `mova_num_experts` in the HFQ config (`source_is_k2_mova`) and generation routes via `GenerationRoute::K2HorizonAr` (bundle-type driven, `LoadedModel::k2_horizon()`). New rdna-compute helpers (`grouped_rmsnorm_f32`, `grouped_rmsnorm_rotate_mq`, `replicate_batched_f32`, `softplus_gate_f32`, `moe_down_combine_silu_overwrite_k8_batched`, `deepseek4_moe_topk_bias_aware_sigmoid_f32`, `gemv_mq4g256v2_residual_noscratch`) + kernels (`grouped_rmsnorm_mq_rotate`, `replicate_batched`, `softplus_gate`, sigmoid topk + batched non-multiple-of-32 fixes, MoVA silu-overwrite combiner). Spec decode is not wired for MoVA — AR only.
- k2-horizon-uno adversarial-review fix pass: **non-greedy penalties** — the sampled verify arms skipped the repeat/presence/frequency law entirely (df290664 fixed greedy only), so the arch-16 serve default (temp 0.3 / top_p 0.8 / repeat_penalty 1.05) decoded a different distribution than AR; the penalty now applies to the committed root row and every verify row (rolling history, same convention the greedy walk verified token-identical), before the top-k support mask so the filter sees the penalised row exactly as AR does. **Tree KV-capacity overflow** — the tree forward wrote one KV row per node with the node budget unclamped by remaining context (a 64-node tree near the context end wrote past `kv.physical_cap`); `step_tree` clamps the node budget to the window budget and `forward_prefill_batch_tree` now errors on `position + rows > physical_cap` like the adapter batch already did. **Prefill first-token law** — the batch path drew its committed first token from the host `Distribution` law (full-vocab nucleus) while every other draw uses the fused AR sampler law (top-20 default cap, inclusive nucleus boundary); it now runs the same device sampler law, including the top-k support mask. **Graph-safety + leaks** — `apply_repeat_penalty_row` used a raw `hip.launch_kernel` with stack kernargs (the dangling-kernarg-under-capture landmine class) and is now `launch_maybe_blob`; `UnoSpeculator::free` destroyed neither captured window `Graph`/`GraphExec` (device memory outliving model unload). **Housekeeping** — `registry/v1.json` restored to generator bytes (the branch had hand-reordered a field the daily regen would revert); dead split-K xbatch kernels + wrappers removed (zero callers since the plain-8-row rewrite); xbatch batch-width bound is release-checked, not `debug_assert`; `HIPFIRE_DUMP_EVENTS` read once per run instead of per event; mangled struct literals reformatted; `PendingWork`/`EngineConfig`/`SubmitRequest` literal rot in three slots examples fixed (`cargo check --workspace --all-targets` is green again); five machine-pathed `scripts/uno_*` probes and the raw `results/` scratch removed — `hipfire bench --model-draft` (added in df290664) is the harness.
- K2-Horizon Uno reference-gap close-out (vs [ifm-ai/uno](https://github.com/ifm-ai/uno)): the Ψ-Spec **tree** verify now takes top-p/top-k requests — upstream's tree path samples every target pick (root and per-node) through the full request `Sampler` (`two_pass_decoding.py`), while hipfire's tree routing excluded nucleus requests (falling to the linear gather verifier) and admitted top-k-only requests with *unfiltered* picks, a law mismatch vs AR. `step_tree` now computes the same `(top_p, top_k)` request law as `step_device`'s nucleus arm and passes it into both `sample_row` draws; routing widens `dev_topk_only` → `dev_filterable` (min-p still refused at admission) and requires batch scratch so host-fallback hardware falls to the host law instead of erroring. Also: the hipGraph window-capture defect is fixed at the mechanism — the capture pass *records* without executing, and the capture branch returned `Ok(())` without running the window, so the capture window sampled the previous window's logits out of the persistent batch buffers (the byte-identical-proposals trace in `docs/perf-checkpoints/2026-09-25-k2-uno-throughput-gfx1101.md` §Defects 1); the freshly instantiated exec is now replayed once for the capture window itself (eager fallback if that first replay fails). Default `HIPFIRE_UNO_GRAPH_CTX` stays 0 pending broader validation. `docs/env-vars.md` corrected (`HIPFIRE_UNO_BLOCK` clamp is 2..=16, `HIPFIRE_UNO_TREE_K` default is 16) and the missing `HIPFIRE_UNO_LORA_SCALE` / `HIPFIRE_UNO_TRACE` / `HIPFIRE_UNO_GRAPH_CTX` rows added. Measured gfx1100 Q8, `k2-horizon-7b.mq4` + K2-Horizon-7B-Uno adapter, `uno_perf_probe` identity: linear greedy PASS (τ 3.000), tree-16 greedy PASS (τ 3.625), capture-on (`HIPFIRE_UNO_GRAPH_CTX=256`) PASS on this build while the pre-change build reproduces the documented divergence ("pos 23 expected 31385, got 200, top2 gap 0.9994") — a rigorous A/B on identical fixtures. End-to-end `hipfire run` temp 1.0 / top-p 0.9: tree-16 τ 1.52 / 67.5 tok/s vs linear τ 1.11 / 62.9 (same decode behavior and speed as the pre-change build; an earlier 20.6 tok/s reading was neighbor-container GPU contention). Deliberate divergence kept: the correction residual `(p−q)+ → 0` edge stays fail-closed (`status=2` → request error) instead of upstream's sparse-path `clean_probs` fallback — unreachable in practice (requires p ≡ q within fp plus an unlucky reject draw) and the kernel header documents the choice; upstream's own fused kernel has no fallback either.
- K2-Horizon Uno greedy spec decode now applies the request's repeat/presence/frequency penalties — a cross-arch correctness defect, not a throughput one. AR scales the logits of tokens inside the repeat window *before* its argmax, and the speculative verifier had no penalty input at all (`SpecRequestConfig` carried temp/top_p/top_k/min_p and nothing for repetition), so a penalised greedy request decoded a different distribution than AR: at the shipped `repeat_penalty` 1.1 the two sides diverged at committed index 5 on the K2 fixture. `SpecRequestConfig` gains `repeat_penalty` + `repeat_window`; `UnoSpeculator` applies AR's Phase-0 law to every committed row — the linear window's draft and verify rows, the tree's per-node rows (each node's history being the common prefix plus its root path), and the root `clean` — and the speculate arms of `ar.rs` consult `nonneutral_penalties`. Verified token-identical to AR on the pinned 527-token prompt at penalties 1.0/1.05/2.0/3.0 (linear) and 1.0/1.05/2.0 (tree-16) against a scalar same-law reference. `hipfire bench` gained `--model-draft` (Conditional-LoRA Uno adapter dir), now honours `--warmups` (it ran one hardcoded "Hello" then measured cold prompt runs), and records per-run `tau`, an all-channels output md5 and token ids in `--json`. Measurement record: `docs/perf-checkpoints/2026-09-25-k2-uno-throughput-gfx1101.md` (promoted from the repo root; the raw `results/` sweep JSON was scratch and is gone — every median and digest lives in the record) — best measured Uno configuration is 1.103x AR (block 2, token-identical), and 2.0x is unreachable with this same-size LoRA drafter.
- K2-Horizon Uno vs AR (gfx1100 200k Q8 greedy HTTP, `Reply with the single word OK.`, 5 runs after warmup): the 8-row window GEMV ran at 4 waves/SIMD so two 8-row forwards at τ≈1.13 lost to 1-token AR (54.8 vs 105.9 tok/s). Default `HIPFIRE_UNO_BLOCK` is now 4 (8-wave xbatch occupancy); LoRA A drops split-K+memset (rank-128 fills without it); short-ctx hipGraph capture/replay of the draft and verify windows (256-token LDS cap, positions[] still the causal bound). Measured same endpoint: Uno B4 65.5 tok/s (τ=0.94), tree-16 69.4 tok/s (τ=1.5), AR 107.0 tok/s. Same-size LoRA draft still cannot beat AR at this TPF; serve default stays AR until it does.
- K2-Horizon Uno vs [ifm-ai/uno](https://github.com/ifm-ai/uno): draft noise now uses the upstream half-open range `[1, vocab)` (K2 mask token 250624); `deterministic_uniform` hashes the prompt and mixes salt / completion count / seed / seq len exactly as `noise.py`; `HIPFIRE_UNO_BLOCK` admits 2..=16 (`max_diffusion_block_size`) with LoRA xbatch chunked at 8; tree candidate top-k defaults to 16; tree verify refuses non-Q8 KV; a ThinkCap/EOS mid-window replays the consumed prefix instead of a 200k re-prefill.
- K2-Horizon Uno close-out: stop tokens come from the model EOS set and the IFM end markers; a ThinkCap/EOS mid-window replays the consumed prefix onto the Llama KV instead of a full re-prefill; grammar and min-p are refused before prefill; assistant history with no thinking field renders an empty `<ifm|think>` block; the MQ4G256V2 x-batch GEMV stays on the K2 prefill arms.
- K2-Horizon Uno gfx1100: the device-resident batch path was gated `gfx1101`+Q8 only, so gfx1100 (same RDNA3 wave32 WMMA ISA) fell to sequential per-token GEMVs and a full 250k-vocab D2H per row — GPU power never saturated and Uno ran at ~0.22× AR. The window carve-out now admits gfx1100/1101/1102/1150/1151/1200/1201 with quantized KV; V2 prefill arms reuse persistent `v2_rot` instead of hipMalloc per layer; window lm_head uses xbatch (n<=8) instead of an underfilled 16-wide WMMA N-tile; default `HIPFIRE_UNO_BLOCK` is 8 (upstream). Host-logit fallback remains for other arches. Measured gfx1100 200k Q8 greedy HTTP (`Reply with the single word OK.`, 5 runs after warmup): 23.6 → 54.8 tok/s (stdev 0.08, τ=1.13), 2.3× the host-logit path; AR on the same endpoint was 105.9 tok/s.
- K2-Horizon Uno: device-resident top-p (nucleus) verification — the vanilla non-greedy regression. A request at temp>0 with top_p in (0,1) (the llama arch-16 serve default is temp 0.3 / top_p 0.8) previously fell to the host `Distribution` law: full 250k-vocab logits downloaded per row per forward (gfx1101 B4 temp1.0 top_p=0.8: 101.8 ms/window / 28.3 tok/s vs greedy 53.3 / 55.5). New `uno_filter_gather` / `uno_filter_finalize` kernels build the per-row p/q laws with the fused AR sampler's exact arithmetic — top-64 logit gather (cap = request top_k, else the sampler's 20), softmax at temp, nucleus truncation at top_p, renormalize — and run acceptance/residual correction on ≤64 candidates; proposals and the bonus draw now pass the request top_p/top_k into `sample_top_p_pf`, so the emitted distribution matches the daemon's non-speculative AR decode at the same request config (the host `Distribution` law was a different, full-vocab-nucleus law). top-k-only and unfiltered paths unchanged; min-p-only requests keep the host law. Measured gfx1101 B4 (3 fresh-process runs): top_p=0.8 temp1.0 101.4->56.5 ms/window / 28.5->47.9 tok/s (+68%), top_p=0.95 50.8 tok/s; top_k=50 38.7, temp1.0 unfiltered 52.6, greedy 55.5 tok/s all unchanged. End-to-end `hipfire serve` at the arch default (vanilla POST, temp 0.3 / top_p 0.8): 48.8 tok/s vs greedy 54.1; temp 0.6 / top_p 0.8: 55.4.
- K2-Horizon Uno: f16 LoRA A/B factors (P2b). The rank-128 conditional-LoRA deltas over 36 layers re-read ~1.36 GB of F32 A+B weights per draft forward — the dominant draft cost (profile: `gemv_f32_xbatch_splitk` 11.8 ms + `gemv_f32_xbatch` 7.2 ms). Storing A/B as F16 halves that read; activations/accumulation stay F32 (new `gemv_f16_xbatch` / `gemv_f16_xbatch_splitk` kernels, `__half` weights, F32 x/y, F32 accumulate; Rust `f16::from_f32` at upload). Measured gfx1101 B4: draft fwd 37.1->32.1 ms, window 57.2->52.5 ms, greedy 51.6->56.4 tok/s (+9.3%), temp1.0 52.4 tok/s, greedy identity PASS (the small delta's logit shift stays inside TIE_EPS 1.0).
- K2-Horizon Uno: device-resident top-k verification. Filtered top-k-only requests now stay on GPU instead of the host `Distribution` law: a new `uno_apply_topk_mask` kernel reduces each draft/verify row to its top-K support (flooring other lanes to a finite `-1e30` the fused verifier's isfinite guard accepts, keeping the top-K ORIGINAL values), then the existing fused dual-softmax reject/correct computes the filtered p/q — upstream `build_sparse_top_k_probs` support. top-p-only and min-p filtering keep the host law (the fused kernel cannot express a nucleus support). Probe knobs `HIPFIRE_UNO_TOP_K` / `HIPFIRE_UNO_TOP_P`. Measured gfx1101 B4 temp1.0: filtered top_k=50 device 78.9 ms/window / 36.5 tok/s vs top_p=0.95 host 101.8 ms/window / 26.3 tok/s (+39%); unfiltered regression unchanged (51.6 tok/s, greedy identity PASS). Known micro-opt (not yet taken): the mask is O(K·vocab); a single-pass top-K heap would bring filtered closer to the unfiltered ~59 ms floor.
- K2-Horizon Uno: draft noise-mode parity with upstream `nano_vllm_uno/engine/noise.py` behind `HIPFIRE_UNO_NOISE` (`random_uniform` default | `deterministic_uniform` | `mask`). `deterministic_uniform` uses the upstream splitmix64 `_mix_u64` finalizer over a per-request stream mixed with the window seed token (reproducible per request, decorrelated per window); `mask` emits constant `noise_high` (= mask-token) rows, kept for parity though out-of-distribution for the adapter. `uno_perf_probe` now reports `tpf` (emitted tokens per 2-forward window) matching upstream TPF accounting; `uno.rs` module header corrected (stochastic verify + overwrite/compact KV, not the stale greedy/replay description). Env knobs `HIPFIRE_UNO_BLOCK` / `HIPFIRE_UNO_TREE` / `HIPFIRE_UNO_TREE_K` documented. Unit tests: noise-mode parse, bounds + replay for random/deterministic laws. No perf delta expected (default mode unchanged).
- K2-Horizon Uno: port the [upstream fused verifier](https://github.com/ifm-ai/uno/blob/main/nano_vllm_uno/engine/fused_verify_kernel.py) to HIP for the existing gfx1101/Q8 batched route. Unfiltered nonzero-temperature verification keeps target logits on device and downloads compact acceptance/correction pairs; proposal and bonus sampling remain on the host. Greedy and filtered requests retain the existing verifier. Request-seeded Philox preserves replay within this implementation, not the previous CPU/Triton token stream. GPU rejection-law and single-turn serving checks pass; no speedup or retained-replay claim. Multi-turn validation remains blocked by K2 assistant-history rendering rejecting a missing thinking field and falling back to Plain.
