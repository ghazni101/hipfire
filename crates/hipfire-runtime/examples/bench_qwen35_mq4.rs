...
fn main() {
    ...
    let use_graph_prefill = std::env::var("HIPFIRE_GRAPH_PREFILL").ok().as_deref() == Some("1");
    ...
    if use_graph_prefill {
        ...
        if b > pbs.max_batch {
            panic!(
                "HIPFIRE_GRAPH_PREFILL=1: prompt b={} exceeds pbs.max_batch={}",
                b, pbs.max_batch
            );
        }
    ...
    if do_profile {
        ...
        if let Some(ref report) = report {
            ...
            if !report.rocprof.is_empty() {
                ...
                eprintln!(
                    "  rocprof total:   {:.1}ms  ({} kernels seen)",
                    report.rocprof_total_us / 1000.0,
                    report.rocprof.len()
                );
                eprintln!(
                    "  internal total:  {:.1}ms  ({} kernels tracked)",
                    report.internal_total_us / 1000.0,
                    entries.len()
                );
                eprintln!(
                    "  blindspot total: {:.1}ms  ({} un-tracked kernels)",
                    report.blindspot_total_us / 1000.0,
                    report.blindspots.len()
                );
                if !report.blindspots.is_empty() {
                    ...
                    for bs in &report.blindspots {
                        eprintln!(
                            ...
                            bs.name,
                            bs.calls,
                            bs.duration_us / 1000.0,
                            bs.percent
                        );
                    }
                    if report.coverage_pct < 90.0 {
                        eprintln!(
                            "  WARNING: coverage {:.1}% < 90% -- investigate blindspot kernels",
                            report.coverage_pct
                        );
                    }
                }
    ...
    if prefill_samples_ms.len() > 1 {
        ...
        eprintln!(
            "  median: {median_ms:.1}ms  {:.1} tok/s",
            prefill_len as f64 / (median_ms / 1000.0)
        );
    }
    ...
    if let Some(ref atlas_path) = atlas_out {
        ...
        if let Some(kernel_ms) = prefill_kernel_ms {
            ...
            if prefill_ms > 0.0 {
                row.set_metric_f64(
                    "cold_overhead_pct",
                    (prefill_ms - kernel_ms) / prefill_ms * 100.0,
                );
            }
        }
        ...
        if let Some(ref report) = prefill_rocprof_report {
            if !report.rocprof.is_empty() {
                let mut rocprof_sorted: Vec<hipfire_atlas::AtlasRocprofKernel> = report
                    .rocprof
                    .iter()
                    .map(|k| hipfire_atlas::AtlasRocprofKernel {
                        name: k.name.clone(),
                        calls: k.calls,
                    ...
                    .collect();
                rocprof_sorted.sort_by(|a, b| b.duration_us.partial_cmp(&a.duration_us).unwrap());
                let blindspots_atlas: Vec<hipfire_atlas::AtlasRocprofKernel> = report
                    .blindspots
                    .iter()
                    .map(|k| hipfire_atlas::AtlasRocprofKernel {
                        name: k.name.clone(),
    ...
    eprintln!(
        "  total: {warmup_ms:.1}ms  avg: {:.2}ms/tok",
        warmup_ms / warmup_len as f64
    );
    ...
    if do_profile_decode {
        if let Some(entries) = rdna_compute::profile::stop() {
            ...
            eprintln!(
                "\n=== DECODE PROFILE ({} launches, {:.1}ms wall) ===",
                entries.len(),
                gen_total_ms
            );
            let mut kerns: Vec<_> = by_kernel.iter().collect();
