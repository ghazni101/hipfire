...
impl Gpu {
    ...
    /// rows*groups grid of `rmsnorm_f32` calls. x and out can alias.
    pub fn rmsnorm_grouped_batched(
        &mut self,
        x: &GpuTensor,
        weight: &GpuTensor,
        out: &GpuTensor,
        ...
        self.bind_thread()?;
        self.ensure_kernel(
            "rmsnorm_grouped_batched",
            kernels::RMSNORM_GROUPED_BATCHED_SRC,
            "rmsnorm_grouped_batched",
        )?;
        ...
        let groups_val = groups as i32;
        let eps_val = eps;
        let mut params: Vec<*mut c_void> = vec![
            &x_ptr as *const _ as *mut c_void,
            &w_ptr as *const _ as *mut c_void,
            &out_ptr as *const _ as *mut c_void,
            &width_val as *const _ as *mut c_void,
            &groups_val as *const _ as *mut c_void,
            &eps_val as *const _ as *mut c_void,
        ];
        let block_size = 256u32.min(width as u32);
        let shared_mem = block_size * 4;
        let bytes = crate::profile::rmsnorm_bytes(rows * width * groups);
        ...
        let result = self.launch_maybe_blob(
            ...
            || {
                ...
                b.push_ptr(w_ptr);
                b.push_ptr(out_ptr);
                b.push_i32(width_val);
                b.push_i32(groups_val);
                b.push_f32(eps_val);
                b
            },
        );
    ...
    pub fn grouped_rmsnorm_f32(
        ...
        let bytes = crate::profile::rmsnorm_bytes(batch * n);
        let timer = crate::profile::begin_timer(&self.hip, "rmsnorm", "grouped_rmsnorm_f32", bytes);
        let result = self.launch_maybe_blob(
            "grouped_rmsnorm_f32",
            [grid, 1, 1],
            [block_size, 1, 1],
    ...
    pub fn replicate_batched_f32(
        ...
        self.ensure_kernel(
            "replicate_batched",
            kernels::REPLICATE_BATCHED_F32_SRC,
        ...
        let total = (batch_size * k_top * k) as i32;
        let mut params: Vec<*mut c_void> = vec![
    ...
    #[cfg(feature = "deltanet")]
    pub fn softplus_gate_f32(&mut self, gate: &GpuTensor, out: &GpuTensor) -> HipResult<()> {
        self.bind_thread()?;
        self.ensure_kernel(
            "softplus_gate",
            kernels::SOFTPLUS_GATE_SRC,
            "softplus_gate_f32",
        )?;
        let gp = gate.buf.as_ptr();
        ...
        let bytes = crate::profile::elementwise_bytes(n as usize);
        let timer =
            crate::profile::begin_timer(&self.hip, "elementwise", "softplus_gate_f32", bytes);
        let result = self.launch_maybe_blob(
            "softplus_gate_f32",
            [grid, 1, 1],
            [block, 1, 1],
