...
impl Gpu {
    ...
    /// F32 x-batched GEMV: `y[b*M+row] (+)= A[row,:] · x[b,:]` for b < `batch`.
    /// `accumulate` folds the result into `y` (LoRA delta onto the base
    /// projection); otherwise plain store.
    pub fn gemv_f32_xbatch(
        &mut self,
        a: &GpuTensor,
        x: &GpuTensor,
        y: &GpuTensor,
        ...
        let timer = crate::profile::begin_timer(&self.hip, "gemv", "gemv_f32_xbatch", bytes);
        let result = self.launch_maybe_blob(
            "gemv_f32_xbatch",
            [m as u32, 1, 1],
            [32, 1, 1],
