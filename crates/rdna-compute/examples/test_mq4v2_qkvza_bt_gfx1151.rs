...
//! MQ4V2 qkvza BT parity on exact gfx1151 — batch-column reuse gate.
//!
...
//! guards are exercised. Distinct NaN sentinels per projection fill outputs
//! before launch so an unwritten tail is caught. On exact gfx1151:
...
//!   2) run the direct `gemm_qkvza_mq4g256v2_wmma_gfx1151_bt` launchers into
...
fn main() {
    ...
    eprintln!("arch {arch} confirmed exact gfx1151 — running qkvza BT parity");
    ...
    // Distinct NaN sentinels per projection (quiet NaN with payload).
    let sentinel_qkv = f32::from_bits(0x7FC0_0001);
    let sentinel_z = f32::from_bits(0x7FC0_0002);
    let sentinel_beta = f32::from_bits(0x7FC0_0003);
    let sentinel_alpha = f32::from_bits(0x7FC0_0004);
    ...
    for &n in &ns {
        ...
        // Fill reference outputs with distinct NaN sentinels to catch unwritten tails.
        for (buf, sen, len) in [
            (&d_y_qkv_ref, sentinel_qkv, n * qkv_m),
            (&d_y_z_ref, sentinel_z, n * z_m),
            (&d_y_beta_ref, sentinel_beta, n * beta_m),
            (&d_y_alpha_ref, sentinel_alpha, n * alpha_m),
        ] {
        ...
        // Sentinel check: base must have overwritten all NaNs.
        for (label, data, sen) in [
            ("qkv_ref", &y_qkv_ref, sentinel_qkv),
            ("z_ref", &y_z_ref, sentinel_z),
            ("beta_ref", &y_beta_ref, sentinel_beta),
            ("alpha_ref", &y_alpha_ref, sentinel_alpha),
        ] {
        ...
        for &bt in &bts {
            ...
            for (buf, sen, len) in [
                (&d_y_qkv_bt, sentinel_qkv, n * qkv_m),
                (&d_y_z_bt, sentinel_z, n * z_m),
                (&d_y_beta_bt, sentinel_beta, n * beta_m),
                (&d_y_alpha_bt, sentinel_alpha, n * alpha_m),
            ] {
                let host = vec![sen; len];
                gpu.hip
                    .memcpy_htod(&buf.buf, unsafe {
            ...
            let res = gpu.gemm_qkvza_mq4g256v2_wmma_gfx1151_bt(
                &d_qkv,
                &d_z,
                &d_beta,
                &d_alpha,
            ...
            for (label, data, sen) in [
                ("qkv_bt", &y_qkv_bt, sentinel_qkv),
                ("z_bt", &y_z_bt, sentinel_z),
                ("beta_bt", &y_beta_bt, sentinel_beta),
                ("alpha_bt", &y_alpha_bt, sentinel_alpha),
            ] {
                let cnt = data.iter().filter(|x| x.to_bits() == sen.to_bits()).count();
                if cnt != 0 {
            ...
            if n == 256 {
                ...
                // Re-fill with sentinels for timing launch as well
                for (buf, sen, len) in [
                    (&d_y_qkv_bt, sentinel_qkv, n * qkv_m),
                    (&d_y_z_bt, sentinel_z, n * z_m),
                    (&d_y_beta_bt, sentinel_beta, n * beta_m),
                ...
                gpu.hip.device_synchronize().unwrap();
                gpu.gemm_qkvza_mq4g256v2_wmma_gfx1151_bt(
                    &d_qkv,
                    &d_z,
                    &d_beta,
                    &d_alpha,
