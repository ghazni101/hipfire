...
//! MQ4V2 residual BT parity on exact gfx1151 — batch-column reuse gate.
//!
...
//! unmissable, and F32 X. Boundary-crossing size m=56 is not 16-aligned
//! (8-row tail) so tail guards are exercised. On exact gfx1151:
...
//!   2) run the direct `gemm_mq4g256v2_residual_wmma_gfx1151_bt` BT4/6/8
...
//!  including nonzero deterministic initial residual. Distinct NaN sentinel
//!  check ensures no sentinel remains. On any other arch the harness SKIPs
//!  cleanly (exit 0, no GPU work).
...
fn main() {
    ...
    eprintln!("arch {arch} confirmed exact gfx1151 — running residual BT parity (Y+=W@X)");
    ...
    // Boundary-crossing: m=56 is not 16-aligned (8-row tail) so tail guards are exercised.
    let m: usize = 56;
    let k: usize = 512;
    assert_eq!(k % 256, 0);
    assert_eq!(m % 16, 8, "m should have 8-row tail");
    ...
    // Distinct NaN sentinel for residual tail check (should never remain after correct Y+=W@X).
    let sentinel_resid = f32::from_bits(0x7FC0_00FF);
    ...
    for &n in &ns {
        ...
        // Fill with sentinel first, then overwrite with nonzero init to ensure init overwrites sentinel.
        {
            let host = vec![sentinel_resid; n * m];
            gpu.hip
        ...
        let cnt_sentinel = y_ref
            .iter()
            .filter(|x| x.to_bits() == sentinel_resid.to_bits())
            .count();
        assert_eq!(cnt_sentinel, 0, "ref still contains sentinel NaN N={n}");
        ...
        for &bt in &bts {
            ...
            let res: Result<(), _> =
                gpu.gemm_mq4g256v2_residual_wmma_gfx1151_bt(&d_a, &d_x, &d_y_bt, m, k, n, bt);
            if let Err(e) = res {
            ...
            let cnt = y_bt
                ...
                .count();
            if cnt != 0 {
                eprintln!("  BT{bt} still contains {cnt} sentinel NaN(s) N={n}");
                all_ok = false;
            }
