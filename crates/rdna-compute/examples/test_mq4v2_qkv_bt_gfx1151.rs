...
//! MQ4V2 qkv BT parity on exact gfx1151 — batch-column reuse gate.
//!
//! Pack deterministic MQ4V2 q/k/v weights with disjoint halves (half0 in
...
//! routing-sensitive (q=40/k=32/v=48 is not 16-aligned; total 120 has an
//! 8-row tail) so straddling tiles and tail guards are exercised. Distinct
//! NaN sentinels per projection fill outputs before launch so an unwritten
//! tail is caught. On exact gfx1151:
//!   1) run the historical base `gemm_qkv_mq4g256v2_wmma` into reference
//!      outputs (force base by arming capture_mode so production BT policy is skipped),
//!   2) run the direct `gemm_qkv_mq4g256v2_wmma_gfx1151_bt` launchers into
//!      separate outputs for N=128 and N=256 with production tiles
//!      BT∈{4,12},
//!  and compare all three projections (q,k,v) for raw f32 bit equality
...
fn main() {
    ...
    eprintln!("arch {arch} confirmed exact gfx1151 — running qkv BT parity");
    ...
    // Routing-sensitive shapes: q=40/k=32/v=48 is not 16-aligned per projection;
    // total 120 has an 8-row tail tile so tail guards are exercised.
    let q_m: usize = 40;
    let k_m: usize = 32;
    let v_m: usize = 48;
    let k: usize = 512;
    ...
    assert_eq!(blob_q.len(), q_m * (k / GROUP) * GROUP_BYTES);
    assert_eq!(blob_k.len(), k_m * (k / GROUP) * GROUP_BYTES);
    assert_eq!(blob_v.len(), v_m * (k / GROUP) * GROUP_BYTES);
    eprintln!(
        "packed q {} B, k {} B, v {} B",
        blob_q.len(),
        blob_k.len(),
    ...
    for &n in &ns {
        ...
        let d_y_q_ref = gpu
            ...
            .expect("alloc y_q ref");
        let d_y_k_ref = gpu
            .alloc_tensor(&[n * k_m], DType::F32)
            .expect("alloc y_k ref");
        let d_y_v_ref = gpu
            .alloc_tensor(&[n * v_m], DType::F32)
            .expect("alloc y_v ref");
        ...
        assert!(is_finite(&y_q_ref), "ref q not finite N={n}");
        assert!(is_finite(&y_k_ref), "ref k not finite N={n}");
        assert!(is_finite(&y_v_ref), "ref v not finite N={n}");
        assert!(variance(&y_q_ref) > 1e-12, "ref q degenerate N={n}");
        assert!(variance(&y_k_ref) > 1e-12, "ref k degenerate N={n}");
        assert!(variance(&y_v_ref) > 1e-12, "ref v degenerate N={n}");
        ...
        for &bt in &bts {
            ...
            let res = gpu.gemm_qkv_mq4g256v2_wmma_gfx1151_bt(
            ...
            if n == 256 {
                ...
                gpu.hip.device_synchronize().unwrap();
                gpu.gemm_qkv_mq4g256v2_wmma_gfx1151_bt(
    ...
    if all_ok {
        eprintln!("\nPASS: all N={{128,256}} x BT{{4,12}} x {{q,k,v}} raw-bit equal (f32::to_bits); finite/nondegenerate; relL2<=1e-5 cosine>=1-1e-6; maxAbs reported per variant");
    } else {
        eprintln!(
