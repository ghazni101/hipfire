    ...
    match source_dtype {
        "F16" => {
            assert_eq!(
                data.len(),
                2 * n_elements,
                "dots-ocr: F16 tensor has {} bytes, expected 2 * {n_elements}",
                data.len()
            );
            data.to_vec()
        }
...
fn load_norm_weight_raw_from_source(
    ...
    let (info, data) = source
        .tensor_data(name)
        .unwrap_or_else(|| panic!("dots-ocr: tensor not found: {name}"));
    // Match the HFQ path's numerical recipe: norms are stored F16 in the HFQ
    // (the quantizer narrows bf16→f16), and `load_norm_weight_raw` widens that
    // F16 back to F32. The dots vision tower is acutely sensitive to per-weight
    // precision (a ~5e-4 norm delta compounds ~1.36×/block over 42 blocks into
    // total divergence), so the from_source path MUST narrow through F16 too —
    // uploading the higher-precision raw bf16→f32 norm seeds the divergence.
    let f32_data = source_bytes_to_f32_via_f16(&info.dtype, data, n);
    gpu.upload_f32(&f32_data, &[n])
}

/// Widen source bytes to F32 *through* F16, matching the HFQ path which stores
/// norms/biases as F16 and widens with `f16_to_f32`. Identical to
/// `source_bytes_to_f32_vec` for F16 sources; for BF16/F32 it first narrows to
/// F16 so the value matches the validated HFQ numerical recipe bit-for-bit.
fn source_bytes_to_f32_via_f16(source_dtype: &str, data: &[u8], n: usize) -> Vec<f32> {
    let f16_bytes = source_bytes_to_f16_stream(source_dtype, data, n);
    f16_bytes
        .chunks_exact(2)
        .map(|c| f16_to_f32(u16::from_le_bytes([c[0], c[1]])))
        .collect()
...
/// Load a bias tensor from ModelSource (always F32 on GPU). Narrows through
/// F16 to match the HFQ path (`load_bias_f32` widens F16-stored biases), so the
/// vision tower sees the same validated numerical recipe — see
/// `load_norm_weight_raw_from_source`.
...
fn load_f16_concat_rows_from_source(
    ...
    assert_eq!(
        bytes_a.len(),
        2 * n_elements_a,
        "dots-ocr: {name_a} produced {} f16 bytes, expected {}",
        bytes_a.len(),
        2 * n_elements_a
    );
    assert_eq!(
        bytes_b.len(),
        2 * n_elements_b,
        "dots-ocr: {name_b} produced {} f16 bytes, expected {}",
        bytes_b.len(),
        2 * n_elements_b
    );
    let mut combined = Vec::with_capacity(bytes_a.len() + bytes_b.len());
...
pub fn load_vision_weights_from_source(
    ...
    for i in 0..cfg.num_hidden_layers {
        if i % 7 == 0 {
            eprintln!(
                "  loading vision block {i}/{} (from source)",
                cfg.num_hidden_layers
            );
        }
        let p = format!("vision_tower.blocks.{i}");
        let norm1_w =
            load_norm_weight_raw_from_source(source, gpu, &format!("{p}.norm1.weight"), h)?;
        ...
        let norm2_w =
            load_norm_weight_raw_from_source(source, gpu, &format!("{p}.norm2.weight"), h)?;
    ...
    // ── post-trunk norm + merger ──────────────────────────────────
    let post_trunk_norm =
        load_norm_weight_raw_from_source(source, gpu, "vision_tower.post_trunk_norm.weight", h)?;
    eprintln!("  loading vision merger (from source)");
    let merger_ln_w =
        load_norm_weight_raw_from_source(source, gpu, "vision_tower.merger.ln_q.weight", h)?;
    ...
    let merger_fc1_b =
        load_bias_f32_from_source(source, gpu, "vision_tower.merger.mlp.0.bias", merge_dim)?;
    ...
    F: FnMut() -> bool,
{
    ...
    for li in 0..cfg.num_hidden_layers {
        ...
        // (v5, n128), fall to F32-K/V (m32, wmma_f32, scalar floor).
        {
            ...
            if let Ok(_variant) = family.resolve(KernelKey::AttnFullF16, &ctx, Some(&shape)) {
