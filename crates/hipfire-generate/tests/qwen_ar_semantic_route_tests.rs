...
use hipfire_generate::{
    common::emit_spec_cancel_after_rollback, qwen::qwen_client_commit_effects,
    qwen::QwenClientCommitEffects,
};
...
fn tool_free_producer_keeps_tool_like_text_as_content() {
    ...
    let text = "<tool_call>\n<function=cat\n</function>\n</tool_call>";
    let mut producer = QwenArSemanticProducer::new_with_tool_protocol("tool-free", false, false);
    let mut sink = Vec::new();
    let mut conversation_tokens = Vec::new();
    let mut streamed_tokens = Vec::new();
    let mut seq_pos = 0usize;
...
fn paired_think_markers_route_reasoning_separately() {
    ...
    assert!(visible.contains(" post") || out.contains(" post") || !fin.trailing_visible.is_empty());
    assert_eq!(fin.finish_reason, "stop");
}
...
fn eos_trailing_marker_prefix_prose_flushed_and_cacheable() {
    ...
    let (out2, visible2, fin2, _, _, _) = drive_ar_semantic_path(&["hi", "<|im_"], false, false);
