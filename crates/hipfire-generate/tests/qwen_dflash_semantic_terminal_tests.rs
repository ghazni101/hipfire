...
fn safe_stop_stores_cache_no_calls() {
    ...
    let term =
        hipfire_generate::qwen::qwen_dflash_wire_terminal(&fin, false, false, "hello", false);
    match &term {
...
fn tool_safe_releases_calls_and_stores() {
    ...
    let term =
        hipfire_generate::qwen::qwen_dflash_wire_terminal(&fin, false, false, "Sure.", false);
    match &term {
        hipfire_generate::qwen::QwenDflashWireTerminal::Done {
            finish_reason,
            release_tool_calls,
...
fn pure_length_suppresses_calls_and_cache() {
    ...
    let term =
        hipfire_generate::qwen::qwen_dflash_wire_terminal(&fin, true, false, "partial", false);
    match &term {
        hipfire_generate::qwen::QwenDflashWireTerminal::Done {
            finish_reason,
            release_tool_calls,
...
fn open_think_prompt_started_and_generated_flags() {
    ...
    for (label, prefix, body) in cases {
        ...
        assert!(
            !hipfire_generate::qwen::qwen_dflash_cache_action(&term).store,
            "{label}"
        );
    }
}
...
fn split_decoded_eot_at_cap_is_stop_not_length() {
    ...
    let term =
        hipfire_generate::qwen::qwen_dflash_wire_terminal(&fin, false, false, &visible, false);
    assert!(matches!(
        term,
        hipfire_generate::qwen::QwenDflashWireTerminal::Done {
            finish_reason: "stop",
...
fn step_budget_max_emit_zero_one_and_mid_window_prefix() {
    ...
    let host_full = hipfire_generate::qwen::spec_host_advance_after_step(
        100,
        0,
        Vec::new(),
        &step.emit,
...
fn forced_token_mid_window_strict_prefix_then_force_advance() {
    ...
    let tx =
        hipfire_generate::qwen::spec_forced_pending_seed_tx(plan.seed_token, &forced_after, true);
...
fn cache_seq_trim_eot_vs_length_body_newline() {
    ...
    assert_eq!(
        hipfire_generate::qwen::qwen_dflash_cache_seq(&body, im_end, &nl),
        body
    );
}
...
fn step_and_forced_advance_error_helpers_are_xor_done() {
    ...
    hipfire_generate::qwen::emit_spec_failure_terminal(
        &mut sink,
        "req-ctx",
        "spec_step",
        "boom",
...
fn decoded_eot_beats_length_cap_helper() {
    ...
    let term =
        hipfire_generate::qwen::qwen_dflash_wire_terminal(&fin_eot, false, false, "hi", false);
    assert!(matches!(
        term,
        hipfire_generate::qwen::QwenDflashWireTerminal::Done {
            finish_reason: "stop",
...
fn cache_fingerprint_uses_visible_not_raw_markers() {
    ...
    assert_eq!(
        f,
        hipfire_generate::common::asst_turn_fingerprint(
...
fn production_done_value_builder_matches_epilogue_shape() {
    let v = hipfire_generate::qwen::qwen_dflash_done_value(
        "r", 3, 1.5, 10, 2.0, 5.0, 1.2, 2.0, 0.5, 2, 0, "length", 99,
    );
...
fn forced_max_tokens_clip_hard_ceiling() {
    ...
    assert_eq!(
        hipfire_generate::qwen::spec_forced_tokens_within_budget(8, 10, &forced),
        &[1, 2]
    );
    assert_eq!(
    ...
    assert_eq!(
        hipfire_generate::qwen::spec_forced_tokens_within_budget(0, 3, &forced),
        &[1, 2, 3]
    );
    assert_eq!(
        hipfire_generate::qwen::spec_forced_tokens_within_budget(9, 10, &forced),
        &[1]
    );
...
fn cancellation_classification_forced_gpu_advance() {
    ...
    match hipfire_generate::qwen::classify_forced_gpu_advance(true) {
        ...
        hipfire_generate::qwen::ForcedGpuAdvanceKind::Committed => {
            panic!("abort must classify Cancelled")
        }
    }
    let out = String::from_utf8(sink).unwrap();
    let lines = parse_jsonl(&out);
...
fn eviction_error_terminal_exclusivity() {
    ...
    assert_eq!(
        hipfire_generate::qwen::classify_evict_failure_wire(),
        hipfire_generate::qwen::SpecFailClosedWire::ErrorOnly
    );
    ...
    match hipfire_generate::qwen::classify_evict_failure_wire() {
        ...
        hipfire_generate::qwen::SpecFailClosedWire::Cancelled => {
            panic!("evict must not classify Cancelled")
        }
    }
    let out = String::from_utf8(sink).unwrap();
    let lines = parse_jsonl(&out);
...
fn strict_prefix_replay_capacity_rejection() {
    ...
    let err_eq =
        hipfire_generate::qwen::spec_prefix_realign_admit(&plan, plan.position, 64, 0, false)
            .unwrap_err();
    assert!(
    ...
    let err_phys =
        hipfire_generate::qwen::spec_prefix_realign_admit(&plan, 5, 64, 0, false).unwrap_err();
    assert!(
    ...
    let err_ctx =
        hipfire_generate::qwen::spec_prefix_realign_admit(&plan, 64, 4, 0, false).unwrap_err();
    assert!(
    ...
    let err_inv =
        hipfire_generate::qwen::spec_prefix_realign_admit(&broken, 100, 100, 0, false).unwrap_err();
    assert!(
    ...
    let err_ev =
        hipfire_generate::qwen::spec_prefix_realign_admit(&plan, 5, 64, 3, true).unwrap_err();
    assert!(
...
fn pending_seed_chain_trigger_clip_force_then_terminal_flush() {
    ...
    let forced =
        hipfire_generate::qwen::spec_forced_tokens_within_budget(host.generated, 3, &forced_raw);
...
fn rollback_attestation_omitted_reset_classes_and_ar_abort_xor() {
    ...
    hipfire_generate::common::emit_spec_cancel_after_rollback(
        ...
        "ar-decode-bad",
        5,
        &attest_epilogue_with_context("ep rank0 bind_thread: bad; device_synchronize failed: x"),
    );
    let out = String::from_utf8(sink).unwrap();
    let lines = parse_jsonl(&out);
...
fn begin_first_token_forced_serviced_before_spec_step() {
    ...
    let clipped = hipfire_generate::qwen::spec_forced_tokens_within_budget(
        generated,
        max_tokens,
        &forced_begin,
    );
    ...
    let max2 = 3usize;
    let forced =
        hipfire_generate::qwen::spec_forced_tokens_within_budget(generated, max2, &forced_begin);
...
fn forced_suffix_stops_at_first_stop_sequence_prefix_only() {
    ...
    let tx = hipfire_generate::qwen::spec_forced_pending_seed_tx(
        incoming_seed,
        &kept,
        incoming_committable,
    );
...
fn first_token_stop_sequence_at_max_tokens_one_is_stop_not_length() {
    ...
    let mut semantic_stop: Option<StopReason> =
        if hipfire_generate::qwen::spec_stop_is_semantic(first_begin.stop) {
            first_begin.stop
        } else {
            None
        };
    ...
    let hit_length = hipfire_generate::common::qwen_dflash_hit_length_cap(
        generated,
        max_tokens,
        decoded_eot,
...
fn held_tool_calls_with_semantic_stop_at_cap_is_tool_calls_not_length() {
    ...
    let term =
        hipfire_generate::qwen::qwen_dflash_wire_terminal(&fin, hit_length, false, "Sure.", false);
    match &term {
        hipfire_generate::qwen::QwenDflashWireTerminal::Done {
            finish_reason,
            release_tool_calls,
    ...
    let length_term =
        hipfire_generate::qwen::qwen_dflash_wire_terminal(&fin, true, false, "Sure.", false);
    match &length_term {
        hipfire_generate::qwen::QwenDflashWireTerminal::Done {
            finish_reason,
            release_tool_calls,
...
fn forced_commit_no_evict_exact_cap_rejects_pending_seed_slot() {
    ...
    assert!(
        !hipfire_generate::qwen::spec_forced_commit_admits(
            position,
            commit_len,
            physical_cap,
            false
