...
pub fn generate_deepseek4_spec(
    ...
    crate::ar::emit_generation_start(
        crate::ar::active_generation_route().unwrap_or(crate::ar::GenerationRoute::Deepseek4Spec),
        stdout,
        id,
...
pub fn generate_deepseek4(
    ...
    // `else` arm of the now-deleted `if spec_mode` spec loop).
    {
        ...
        crate::ar::emit_generation_start(
            crate::ar::active_generation_route().unwrap_or(crate::ar::GenerationRoute::Deepseek4Ar),
            stdout,
            id,
            !matches!(think_mode, ThinkMode::NonThink),
        );
...
pub fn generate_maple(
    ...
    crate::ar::emit_generation_start(
        crate::ar::active_generation_route().unwrap_or(crate::ar::GenerationRoute::MapleAr),
        stdout,
        id,
        primed_think,
    );
...
// ─── K2-Horizon (arch_id=16) generate path ─────────────────────────────
//
...
/// K2-Horizon generate path (arch_id=16). Sequential decode, CPU-side
...
pub fn generate_k2_horizon(
    ...
    if m.k2_horizon().is_none() {
        emit_error_with_id(
            stdout,
            id,
            "k2_horizon bundle missing on arch_id=16 generate",
        );
        let _ = stdout.flush();
        return;
    }
