...
def main() -> int:
    args0 = sys.argv[1:]
    if args0 and args0[0] in ("-h", "--help"):
        print(
            "usage: python3 -m tools.bench.quant_compare_top5 "
            "<baseline-prefix> <mode=prefix>... [--wide-margin N]"
        )
        return 0
    if len(sys.argv) < 3:
        print(
            "usage: python3 -m tools.bench.quant_compare_top5 <baseline-prefix> <mode=prefix>... [--wide-margin N]",
            file=sys.stderr,
        )
        return 2
