> **Lifecycle:** historical plan / follow-up list (archived 2026-08).
> **Authority:** planning archaeology only — not the live roadmap.
> **Provenance:** moved from root `NEXT-STEPS.md` during repository-organization cleanup (2026-08).
> **Live follow-ups promoted to issues:**
> - [#647](https://github.com/warpfront/hipfire/issues/647) — adaptive KV remaining work
> - [#648](https://github.com/warpfront/hipfire/issues/648) — EP constructor-mid-failure VRAM leak
>
> Items marked **historical** below (obsolete TS config-meta gap; branch-local
> archaeology such as the deferred 2E revert) are retained for provenance and are
> not active work.
...
## Immediate follow-ups (own PRs) — *(promoted → issue #647 unless noted)*
...
1. **DFlash / spec-decode hook.** v1 wires only the linear `generate` decode
   ...
   [docs/VALIDATION.md](../../VALIDATION.md) and eyeball the decoded output for
...
8. **`mtp_mode` / `mtp_k` config-meta gap.** *(historical — obsolete TS config-meta
   archaeology; not active adaptive-KV work.)* `cli/config_meta.test.ts` fails
...
9. **Revert 2E (carried over from the V-quant line).** *(historical — branch-local
   archaeology / null-result deferred revert; not live roadmap.)* Commit 373d0f59
...
## EP serving — constructor-mid-failure VRAM leak (scoped follow-up) — *(promoted → issue #648)*
