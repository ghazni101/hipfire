> **Lifecycle:** historical investigation / manual-review queue (archived).
> **Authority:** discovery and dated evidence context only — not the live product surface.
> **Provenance:** moved from root `MANUAL_REVIEW.md` during repository-organization cleanup (2026-08).
> **Note:** The long-context PFlash score NaN cascade below was mitigated by the shipped
> shallow score-layer workaround (`DrafterModel::score_layer_idx` / first FullAttn K) in
> `crates/hipfire-pflash`. Treat open items here as historical escalation notes unless
