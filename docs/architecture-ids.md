...
| 16 | K2-Horizon (MoVA-36B-A4B) | `hipfire-arch-k2-horizon` | `K2HorizonCarrier` | MoVA attention (64 value experts, top-4, sigmoid-routed via `self_attn.v_router` + bias, softplus post-attn gate) + sigmoid MoE FFN (100 experts top-8 + 1 shared, `mlp.gate` router). Grouped RMSNorm (`layernorm_num_groups` 2), full RoPE, GQA 32:8 hd 128, dense layers 0-2 (`mlp_only_layers`). AR-only — spec decode, continuous batch, `pp>1`, `ep` refused. `model_type k2_horizon` → 16. |
